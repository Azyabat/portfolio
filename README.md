...
При первом открытии страницы клиентский `VisitTracker` отправляет событие в `POST /api/visit`, а серверный обработчик отправляет Telegram-уведомление с временем посещения, страницей, IP, источником перехода, браузером, устройством, ОС и геолокацией по IP, если ее удалось определить. Чтобы не спамить при переходах по страницам, трекер ставит флаг `azacode_visit_logged` в `sessionStorage`.

Внутренние визиты с локальными IP (`127.*`, `10.*`, `192.168.*`, приватные `172.*`) игнорируются. Если в уведомлениях появляется `127.0.0.1`, нужно проверить, что nginx/HAProxy прокидывает реальный адрес клиента в `X-Forwarded-For` или `X-Real-IP`.
